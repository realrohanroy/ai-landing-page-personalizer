...
if st.button("⚡ Generate Personalized Page", type="primary", use_container_width=True):
    ...
    with st.spinner("Extracting ad intent and personalizing page..."):
        try:
            ...
            # Clear any previously applied variant when regenerating
            st.session_state.pop("applied_variant", None)
...
# ── Apply variant overlay if one was selected ─────────────────────
if "applied_variant" in st.session_state:
    variant = st.session_state["applied_variant"]
    data["personalized"] = {
        "headline": variant.get("headline"),
        "subheadline": variant.get("subheadline"),
        "cta": variant.get("cta")
    }
    st.info(f"🧪 Showing: **{variant.get('label')}** — {variant.get('strategy')}")
...
if ad_intent:
    ...
    for i, (label, icon) in enumerate(zip(labels, icons)):
        with cols[i]:
            ...
            st.markdown(
                f"<div style='font-size:14px;color:#e2e8f0;padding:8px 0'>{val}</div>",
    ...
    for i, variant in enumerate(variants):
        ...
        with c2:
            if st.button(f"Apply Variant {i+1}", key=f"variant_{i}"):
                st.session_state["applied_variant"] = variant
                st.rerun()  # ← key fix, keeps result in session state
        st.divider()
...
if mode == "full_page":
    ...
    st.caption("Showing hero section reconstruction — full page blocked by site security policy")
if preview_url:
